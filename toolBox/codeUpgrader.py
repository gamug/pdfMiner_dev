            ...
            for file in os.listdir(os.path.join(workPath, folder)):
                if len(file.split('.')) > 1:
                    ...
                    for fileaux in os.listdir(os.path.join(workPath, folder, file)):
                        extention = fileaux.split('.')[len(fileaux.split('.')) - 1]
                        if fileaux in filestoolbox and extention == 'py':
                            shutil.copy2(os.path.join(workPath, folder, 'toolBox', file),
                                         os.path.join(interp_path, 'toolBox'))
                        if file in filesinterface and extention == 'py':
                            shutil.copy2(os.path.join(workPath, folder, 'appInterface', file),
