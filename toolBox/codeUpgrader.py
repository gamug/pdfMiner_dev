    ...
    for folder in os.listdir(workPath):
        ...
        if len(folder.split(r'.')) == 1:
            ...
            for file in os.listdir(os.path.join(workPath, folder)):
                print(os.listdir(workPath,folder,file))
                if len(file.split('.')) > 1:
                    ...
                    for fileaux in os.listdir(os.path.join(workPath, folder, file)):
                        ...
                        if fileaux in filestoolbox and extention == 'py' and folder=='toolBox':
                            shutil.copy2(os.path.join(workPath, folder, file, fileaux),
                                         os.path.join(interp_path, folder))
                        if file in filesinterface and extention == 'py' and folder=='appInterface':
                            shutil.copy2(os.path.join(workPath, folder, file, fileaux),
                                         os.path.join(interp_path, r'appInterface'))
