...
class textMiner():
    ...
    def placeWidgets(self):
        ...
        self.state = scrolledtext.ScrolledText(
            self.frame2, undo=True, relief=RIDGE, bg='black', fg='white',height=self.height, padx = 20, pady = 20,
        ...
        self.edpEval = LabelFrame(self.frame1, text=r'edpEvaluation', bg='black', relief=RIDGE,
                                    height=int(self.height / 3), width=int(self.width / 6.4), fg='white',
                                    font=("comic sans ms", 13, "bold"))
        self.edpEval.pack_propagate(False)
        self.evaluate = Button(self.edpEval, command=self.edpEvalu,
                                 bg='Black', fg='white', font=("comic sans ms", 10), text='evaluateDatabase')


        # Packing all
        self.FpdfMiner.pack(side=TOP)
        self.BpdfMiner1.grid(row = 0, column = 1)
        self.BpdfMiner2.grid(row = 1, column = 1)
        self.FpdfMiner.grid_columnconfigure(1, pad = 40)
        self.FpdfMiner.grid_rowconfigure(1, pad = 40)
        self.edpEval.pack(side = TOP)
        self.evaluate.grid(row=0, column=1)
        self.edpEval.grid_columnconfigure(1, pad=40)
        self.edpEval.grid_rowconfigure(1, pad=40)
    ...
    def edpEvalu(self):
        self.setState('********************************************')
        self.setState('This feature is still under development')
        self.setState('********************************************')
